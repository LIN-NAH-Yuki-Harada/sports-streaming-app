#!/usr/bin/env python3
"""融資依頼書 v2 — 公庫宛 350 万円（協調融資の片翼）

変更点 (v1 → v2):
- 単独 700 万 → 公庫 350 + 埼玉りそな 350 = 協調融資 700 万 (公庫宛は 350 万)
- 開発状況 92% → 本番稼働 100% + 5/10 までに 126 PR マージ
- ドメイン live-spotch.com に統一 (旧 sports-streaming-app.vercel.app)
- ローンチ時期 2026/6〜7月 → 2026/6/1 正式ローンチ
- 着金タイミング: 2026/5/末
- 残作業を「Google OAuth 認定 + TCP 化 (B 案) + 細部仕上げ」に
- 資金内訳を v2 配分に (開発費 210 / マーケ 240 / 運転 170 / 予備 50 / インフラ 30)
- 5/10 試合実機運用実績を強調 (ブロックシード大会 53 分配信完走 + 同日 5 hotfix)
"""
