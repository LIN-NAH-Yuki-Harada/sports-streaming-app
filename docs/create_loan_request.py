#!/usr/bin/env python3
"""融資依頼書 v3 — 公庫宛 350 万円（協調融資の片翼）

変更点 (v2 → v3 / 2026-05-29 改訂):
- 提出スケジュール再設計: 5/29 本依頼書作成 → 6/1 税理士確認 → 6 月上旬 銀行ご提出 → 6 月中旬 着金・正式ローンチ + 広告開始
- ローンチ時期 2026/6/1 → 2026/6 中旬 (税理士確認・銀行審査ステップを織り込み)
- 着金タイミング 2026/5 末 → 2026/6 中旬
- 5 月完璧化進捗を反映: OAuth 通過 (5/13)・YouTube 埋め込み許可・モバイル遷移体感改善・依存ライブラリ最新化 (5/28-29) を完了として記載
- PR マージ件数を最新化 (5/10 時点 126 件 → 5/29 時点 127 件)
- ローンチ前の実課金トラクション (5/21 確認) は v2 で反映済を継承
- KPI ゲートを 2 週後ろ倒し (7 月末→8 月末・8 月初→10 月初)
"""
...
body('日本政策金融公庫　御中', space_after=12)
...
body('先日は新規融資のご相談に対し、貴重なフィードバックをいただき誠にありがとうございました。')
body('ご助言を踏まえ、本依頼書では、貴庫様 350 万円 + 埼玉りそな銀行様 350 万円の合計 700 万円の協調融資としてご検討いただきたく、改めて下記の通りお願い申し上げます。', space_after=12)
...
body('当社の新規事業として、ローカルスポーツ向けライブ配信プラットフォーム「LIVE SPOtCH（ライブ スポッチ）」の運営・マーケティング・運転資金として、貴庫様より金 3,500,000 円の融資を申し込みます。', first_indent=0.5, space_after=6)

bullet('協調融資の構成：本件は貴庫様 350 万円 + 埼玉りそな銀行様 350 万円 = 合計 700 万円の協調融資としてご検討をお願いいたします。埼玉りそな銀行は当社の既存取引銀行であり、別途同行へも 350 万円の融資依頼を進めております。', space_after=3)
...
bullet('スケジュール：2026年5月29日 本依頼書作成 → 6月1日 顧問税理士確認・最終化 → 6月上旬 貴庫様へご提出 → 6月中旬 ご審査・着金（協調融資 700 万円）→ 6月中旬 正式ローンチ + 広告運用開始 → 8月末 KPI ゲート 1（有料 5 名）→ 10月初 KPI ゲート 2（有料 20 名）。6月中旬ローンチ時点で広告運用を開始する計画のため、6月上旬の本申込時点で早期審査をお願い申し上げます。', space_after=12)
...
bullet('返済額（貴庫様 350 万円分）：毎月　約 61,347 円（年利 2.0% 想定時）', space_after=3)
...
bullet('補足：新規事業が計画を下回った場合でも、既存のマーケティングデザイン事業の収入により返済を継続する体制を維持いたします。役員報酬の調整余力もございます。', space_after=12)
...
print(f"保存完了: {out}")
print(f"")
print(f"=== 文書概要 (v3 改訂版 / 2026-05-29) ===")
print(f"宛先: 日本政策金融公庫")
print(f"申込金額: 350 万円 (協調融資 700 万円のうち公庫分)")
