...
c.setFont(JP, 9)
c.drawString(40, y - 72, "スポーツ少年団・部活・地域大会など、テレビでは配信されないローカル試合を、")
c.drawString(40, y - 86, "保護者がスマホ1台で撮影 → 共有URLをタップした人だけがリアルタイム観戦。")
c.drawString(40, y - 100, "アプリのダウンロードは不要。視聴は完全無料です。")
...
steps = [
    ...
    ("STEP 4", "YouTube連携", "チームプラン(¥500/月)なら\nYouTube Live同時配信+アーカイブ\n自動保存。チーム運営も可。"),
]
step_w = (W - 80 - 18) / 4
step_h = 88
sy = y - step_h - 10
...
vsteps = [
    ("STEP 1", "共有URLを受け取る", "LINEで届いたURLをタップ、\nまたはホーム画面で6桁コードを入力。"),
    ("STEP 2", "すぐに観戦スタート", "アプリのダウンロード不要。\nブラウザでログイン → リアルタイム視聴。"),
]
v_step_w = (W - 80 - 8) / 2
v_step_h = 60
vy = y - v_step_h - 10
...
trust = [
    ("限定公開", "共有URLを知る人だけが視聴。\n不特定多数には公開されません。"),
    ("リアルタイム視聴", "撮影と同時に視聴者にお届け。\n試合の流れをその場で共有。"),
    ("TV中継品質", "スコアボードを常時表示。\n試合状況がひと目でわかる。"),
    ("Webだけで完結", "アプリDL不要。LINEで送るだけ。\nどんな端末でも視聴できます。"),
    ("スケジュール", "試合予定を登録してチームで共有。\n配信日もワンタップで。"),
    ("安心のセキュリティ", "Stripe決済・2FA対応・Made for Kids 対応。\n子どもの映像を守る限定配信。"),
]
t_w = (W - 80 - 30) / 6
t_h = 50
ty = y - t_h - 10
