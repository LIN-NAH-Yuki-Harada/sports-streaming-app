...
async function burnScoreboard(recPath, b, events, workDir, idx) {
  ...
  // ffmpeg の入力とフィルタを組み立てる。
  //
  // ★2026-08-11: メモリがイベント数に比例して増える問題を解消した。
  //
  // 【旧方式の問題】得点1件につき「フルスクリーンPNGを1枚 + ffmpeg入力を1本 + overlayを1段」を
  //   鎖状に繋いでいた。実測ピークRSS:
  ...
  //   入力もフィルタも件数に依存しなくなる＝**メモリがイベント数に依存しない**。
  //
  // ★失敗したら旧方式へフォールバックする（下の catch）。最悪でも現行と同じ挙動に留める。
  const outPath = path.join(tmpdir, "scored.mp4");

  // 旧方式（フォールバック用）: N入力 + N段 overlay
  const buildChainedArgs = () => {
    const a = ["-y", "-i", recPath];
    ...
    return { args: a, fc: fc.replace(/;$/, "") };
  };

  // 新方式: concat デマクサで1入力にまとめる。
  // ★最初の得点までの空白区間は「完全に透明なPNG」で埋める（そこはスコアボードを出さない）。
  // ★concat デマクサは最後の file の duration を無視するため、末尾の1枚を2回書くのが定石。
  const buildConcatArgs = () => {
    const blankPng = path.join(tmpdir, "blank.png");
    const entries = [];
    if (segs[0].s > 0.05) {
      entries.push({ png: blankPng, dur: segs[0].s });
    }
    for (let i = 0; i < segs.length; i++) {
      // 隙間（イベント間に間が空くケース）も直前の表示を維持する＝実挙動と同じ
      const next = i + 1 < segs.length ? segs[i + 1].s : segs[i].e;
      const dur = Math.max(0.04, next - segs[i].s);
      entries.push({ png: pngs[i], dur });
    }
    const listPath = path.join(tmpdir, "overlay.txt");
    let list = "";
    for (const e of entries) {
      list += `file '${e.png.replace(/'/g, "'\\''")}'\nduration ${e.dur.toFixed(3)}\n`;
    }
    // 末尾をもう一度（concat デマクサの仕様。これが無いと最後の1枚が一瞬で消える）
    list += `file '${entries[entries.length - 1].png.replace(/'/g, "'\\''")}'\n`;
    fs.writeFileSync(listPath, list);
    return {
      args: ["-y", "-i", recPath, "-f", "concat", "-safe", "0", "-i", listPath],
      // shortest=1 で本編の長さに合わせる（画像側が長くても伸びない）
      fc: "[1:v]format=rgba,setpts=PTS-STARTPTS[ov];[0:v][ov]overlay=0:0:shortest=1[vout]",
      blankPng,
    };
  };
  ...
  let plan;
  try {
    plan = buildConcatArgs();
    // 透明PNG（1枚だけ）を用意する。最初の得点までの区間に使う。
    await spawnP("ffmpeg", [
      "-y", "-f", "lavfi", "-i", `color=c=black@0.0:s=${w}x${h}:d=0.1,format=rgba`,
      "-frames:v", "1", plan.blankPng,
    ]);
  } catch (e) {
    log(`seg${idx}: concat 方式の準備に失敗 -> 旧方式へ: ${String(e).slice(0, 120)}`);
    plan = buildChainedArgs();
  }
