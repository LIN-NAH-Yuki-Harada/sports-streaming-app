...
// 視聴者からのライブ応援スタンプ（❤️/👍）の受信表示は、視聴画面と共通の
// LiveReactions コンポーネント（components/LiveReactions.tsx・右端上昇）に統一した。
// ※ 以前はここに中央寄り(6〜68%)の独自実装があったが、「プレー映像にかぶる」
//   実戦FB（2026-07-11）を受けて右端固定に変更。
...
function ScoreControls(props: ScoreControlsProps) {
  ...
  // Android のバック操作（ボタン/ジェスチャー）で配信が黙って中断しないよう、
  // 配信中（＝このオーバーレイ表示中）は停止確認ダイアログに差し替える。
  // iOS にはバックキーが無いため no-op。
  useEffect(() => {
    const sub = BackHandler.addEventListener("hardwareBackPress", () => {
      confirmStop();
      return true; // 既定のアプリ終了/戻る遷移を抑止
    });
    return () => sub.remove();
  }, [confirmStop]);
  ...
  return (
    <>
      ...
      {/* 視聴者からの応援スタンプ（受信表示のみ・右端上昇・操作を邪魔しない） */}
      {shareCode ? <LiveReactions shareCode={shareCode} /> : null}
    </>
  );
}
