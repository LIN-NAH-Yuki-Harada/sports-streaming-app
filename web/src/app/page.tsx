...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "2つの準備が必要です。①マイページでご自身の YouTube アカウントを連携してください（初期状態はOFFのため、未設定のままだと録画は保存されません）。②15分を超える試合を保存する場合は、その YouTube チャンネルで電話番号の確認（youtube.com/verify・数分で完了・身分証不要）を済ませてください。未確認のチャンネルは YouTube 側が15分超の動画を受け付けないため、アップロードを中止します。サッカー・バレーなど多くの試合は15分を超えるため、初回配信の前にこの2つをご確認ください。なお、パソコン・スマホのブラウザから配信する場合は、上記に加えてYouTube側で「ライブ配信」が使える状態になっている必要があります（初回は使えるようになるまで最大24時間）。アプリから配信する場合、この手続きは不要です。",
  },
  {
