...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const isIOSSafari =
      isIOS && /Safari/.test(ua) && !/CriOS|FxiOS|EdgiOS|OPiOS/.test(ua) && !isInApp;
    const isAndroidChrome =
      isAndroid && /Chrome/.test(ua) && !/SamsungBrowser|UCBrowser|EdgA|OPR/.test(ua) && !isInApp;
  ...
  return (
    <div>
      ...
      {/* ホーム画面追加の誘導（PWAモード・PCでは非表示） */}
      {mounted && !isStandalone && guideKind !== "desktop" && (
        <section className="px-5 md:px-8 lg:px-10 pt-6">
            ...
            <div className="relative">
              <div className="flex items-center gap-3 mb-2">
                <div className="w-11 h-11 rounded-xl bg-[#e63946] flex items-center justify-center shadow-lg shadow-[#e63946]/20">
                  <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v12m-4-8l4-4 4 4M5 15v3a2 2 0 002 2h10a2 2 0 002-2v-3" />
                  </svg>
                </div>
                <div>
                  <p className="text-base font-bold">もっと快適に配信・視聴</p>
                  <p className="text-xs text-gray-500">ホーム画面に追加してアプリ化</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                <div className="rounded-lg bg-black/30 px-2 py-2.5 text-center">
                  <svg className="w-5 h-5 text-[#e63946] mx-auto mb-1" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              ...
              {(guideKind === "ios-needs-safari" || guideKind === "android-needs-chrome") && (
  ...
  return (
    <span className="inline-flex items-center justify-center w-6 h-6 rounded bg-white/10 align-middle mx-1">
      <svg className="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 16V4m0 0l-4 4m4-4l4 4M6 14v4a2 2 0 002 2h8a2 2 0 002-2v-4" />
      </svg>
    </span>
  );
}
...
function IOSInstallSteps() {
  return (
    <div className="mt-5 pt-4 border-t border-white/5">
      <p className="text-xs text-[#e63946] font-bold mb-3">iPhone での追加手順</p>
...
function AndroidInstallSteps() {
  return (
    <div className="mt-5 pt-4 border-t border-white/5">
      <p className="text-xs text-[#e63946] font-bold mb-3">Android での追加手順</p>
      <ol className="space-y-3">
        <li className="flex gap-3">
          <StepNumber n={1} />
          <div>
